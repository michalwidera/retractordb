...
namespace rdb
{
    ...
    template <class T> posixPrmBinaryFileAccessor<T>::posixPrmBinaryFileAccessor(std::string fileName) : fileNameStr(fileName)
    {
        fd = ::open(fileNameStr.c_str(), O_RDWR | O_CREAT | kOpenBaseFlags, 0644);
    };

    template <class T> posixPrmBinaryFileAccessor<T>::~posixPrmBinaryFileAccessor()
    {
        ::close(fd);
    }
