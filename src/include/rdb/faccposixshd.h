...
/// - zapisywać zmiany uprzednio zarejestrowanych danych do pliku cienia (shadow file) zamiast aktualizacji głównego pliku, aby
/// uniemożliwić fizyczną modyfikację uprzednio zarejestrowanych danych.
/// - przechowywać w pliku cienia wyłącznie pary (pozycja, dane) dla operacji update, aby plik cienia był kompaktowy i znacznie
/// mniejszy od głównego pliku danych.
/// - udostępniać dane, jeśli istnieją z pliku cienia jako aktualny stan danych, umożliwiając odczyt i dalsze aktualizacje bez
/// modyfikacji głównego pliku.
/// - w przypadku wielokrotnych aktualizacji tej samej pozycji, zwracać dane z ostatniego zapisu (przeszukiwanie pliku cienia od
/// końca).
...
/// - zapewnić mechanizm integracji pliku cienia z głównym plikiem (merge), umożliwiając scalanie zmian z pliku cienia do
/// głównego pliku na żądanie.
/// - zapewnić mechanizm zarządzania plikiem cienia, umożliwiając usuwanie pliku cienia po scaleniu zmian z głównym plikiem
/// (truncate po merge).
