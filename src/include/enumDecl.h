...
namespace rdb {
BEGIN_E_GEN(eType){DECL(BAD),      DECL(BYTE),  DECL(INTEGER), DECL(UINT),
                   DECL(RATIONAL), DECL(FLOAT), DECL(STRING),  DECL(BYTEARRAY),
                   DECL(INTARRAY), DECL(DOUBLE)} END_E_GEN(eType);
}
