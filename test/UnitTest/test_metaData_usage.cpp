// Scenariusze użycia klasy metaData
//
...
// ---------------------------------------------------------------------------
// Scenariusz 8: Maszyna detekcji gap (przejęta od storage)
//
// Po configureGapDetection(nullFillCount) klasa sama decyduje, które rekordy
...
TEST_F(UsageFixture, scenariusz_maszyna_gap) {
  rdb::metaData meta(descriptor, file);
  EXPECT_FALSE(meta.gapDetectionEnabled());

  meta.configureGapDetection(2);  // 2 rekordy nullfill przed oznaczeniem gap
  EXPECT_TRUE(meta.gapDetectionEnabled());

  // Faza nullfill: pierwsze 2 rekordy all-null przechodzą do zapisu.
  EXPECT_FALSE(meta.absorbAppend(allNull));
  meta.onRecordAppended(allNull);
  EXPECT_FALSE(meta.absorbAppend(allNull));
  meta.onRecordAppended(allNull);

  // Faza gap: kolejne rekordy all-null są pochłaniane (nie trafiają do magazynu).
  EXPECT_TRUE(meta.absorbAppend(allNull));
  EXPECT_TRUE(meta.absorbAppend(allNull));
  EXPECT_EQ(meta.totalRecords(), 2U);  // pochłonięte rekordy nie są indeksowane

  // Rekord nie-null: oczekująca przerwa (2 interwały) zapisana jako wpis gap przed nim.
  EXPECT_FALSE(meta.absorbAppend(allPresent));
  meta.onRecordAppended(allPresent);

  EXPECT_TRUE(meta.isGapBefore(2));
  EXPECT_EQ(meta.totalRecords(), 3U);

  // Wpis gap widoczny w segmentach z zaakumulowaną długością przerwy.
  auto segs   = meta.segments();
  auto gapSeg = std::ranges::find_if(segs, [](const auto &s) { return s.isGap; });
