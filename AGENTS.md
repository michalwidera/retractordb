...
4. Resolve conflicts between sources using the skill's *Source precedence* section, which is the only copy of that ordering.

## Installing the RetractorDB skill

The sibling `knowledge-index` checkout is discovered through the repository symlink. To make the same skill available from the documentation repositories and derived-artifact workspaces, run:

```bash
scripts/install-codex-skill.sh
```

The installer delegates to `../knowledge-index/scripts/install-skill.sh` and creates `~/.agents/skills/retractordb-system` as a symbolic link to the knowledge-index checkout. It is idempotent and refuses to replace an existing file, directory, or link to a different target.
